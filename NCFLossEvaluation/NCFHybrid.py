import torch
from utils import get_parser, translate_args, get_train_instances, validate, TOP1, BPR
...
def main():
    ...
    dataset, dthreshold, model, emb_size, layers, optimizer = translate_args(args)
    ...
    pair_loss_function = None
    if args.loss_fn:
        if args.loss_fn == 'BCE':
            point_loss_function = nn.BCELoss()
            alpha = 0
        if args.loss_fn == 'BCEWithLogitsLoss':
            point_loss_function = nn.BCEWithLogitsLoss()
            alpha = 0
        if args.loss_fn == 'KLDiv':
            point_loss_function = nn.KLDivLoss()
            alpha = 0
        if args.loss_fn == 'MSE':
            point_loss_function = nn.MSELoss()
            alpha = 0
        ...
        if args.loss_fn == 'BPR':
            point_loss_function = nn.BCELoss()
            pair_loss_function = BPR
    else:
        point_loss_function = nn.BCELoss()
        pair_loss_function = BPR

    print('alpha ', alpha)
    print('functions ', point_loss_function, pair_loss_function)
    ...
    for epoch in range(args.epochs):
        ...
        if hr > best_hr:
            ...
            if args.save_best:
                ...
                torch.save(best_model.state_dict(), './saved_models/' + str(model_name) + '.pth')
